  ...
  function add_concert(){
    ...
    $.ajax({
        ...
        if (msg == ''){
            ...
            $('#exampleModalCenter a').attr('href','/result?id=' + msg.id)
...
function search(){
    ...
    data = {};
    if (getUrlParameter('free_id') != ''){
        data = {concert_id : getUrlParameter('free_id')};
    }
    ...
    $.ajax({
        ...
        if (!Array.isArray(results)){
            ...
            for (i=0; i < msg.artists.length;i++){
                ...
                h += '<button type="button" class="btn btn-outline-warning ml-2" artistid="' + msg.artists[i].artist_id + '">' + msg.artists[i].artist_name + '</button>';
            }
            msg.artists_links = h;
        }
        else
        ...
        $('#concertDetails #concertTitle').text(msg.name);
